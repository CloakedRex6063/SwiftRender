...
Swift::D3D12::Resource::Resource(ID3D12Resource *resource) : m_resource(resource)
{
    m_mapped = false;
}
...
Swift::D3D12::Resource::Resource(ID3D12Device14 *device, const TextureCreateInfo &info)
{
    ...
    const auto sample_desc = info.msaa
        ? DXGI_SAMPLE_DESC{info.msaa->samples, info.msaa->quality}
    ...
    [[maybe_unused]]
    const auto result = device->CreateCommittedResource(&heap_properties, D3D12_HEAP_FLAG_NONE,
                                                        &resource_info,
                                                        D3D12_RESOURCE_STATE_COMMON, p_clear_value,
...
Swift::D3D12::Resource::Resource(ID3D12Device14 *device, const BufferCreateInfo &info)
{
    ...
    if(info.flags & BufferFlags::eReadback)
    {
...
uint64_t Swift::D3D12::Resource::GetVirtualAddress()
{
