...
Swift::D3D12::Resource::Resource(ID3D12Device14* device,
                                 ...
                                 const uint32_t offset)
{
    if (heap)
    {
        m_resource = CreatePlacedResource(device, heap, offset, info);
    }
    else
    {
        m_resource = CreateCommittedResource(device, info);
    }
    const auto name = std::wstring{info.name.begin(), info.name.end()};
    m_resource->SetName(name.c_str());
}

Swift::D3D12::Resource::Resource(ID3D12Device14* device, const BufferCreateInfo& info, ID3D12Heap1* heap, const uint32_t offset)
{
    if (heap)
    {
        m_resource = CreatePlacedResource(device, heap, offset, info);
...
D3D12_RESOURCE_DESC Swift::D3D12::Resource::GetResourceDesc(const BufferCreateInfo& info)
{
    return {
...
D3D12_RESOURCE_DESC Swift::D3D12::Resource::GetResourceDesc(const TextureCreateInfo& info)
{
...
ID3D12Resource* Swift::D3D12::Resource::CreatePlacedResource(ID3D12Device14* device,
                                                             ID3D12Heap1* heap,
                                                             const uint32_t offset,
...
ID3D12Resource* Swift::D3D12::Resource::CreateCommittedResource(ID3D12Device14* device, const BufferCreateInfo& info)
{
    const auto resource_info = GetResourceDesc(info);
...
ID3D12Resource* Swift::D3D12::Resource::CreateCommittedResource(ID3D12Device14* device, const TextureCreateInfo& info)
{
    const auto resource_info = GetResourceDesc(info);
